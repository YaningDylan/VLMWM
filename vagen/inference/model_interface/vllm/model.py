...
class VLLMModelInterface(BaseModelInterface):
    ...
    def __init__(self, config: VLLMModelConfig):
        ...
        if hasattr(config, 'max_model_len') and config.max_model_len is not None:
            model_kwargs["max_model_len"] = config.max_model_len
            logger.info(f"Setting max_model_len to {config.max_model_len}")
    
        if hasattr(config, 'max_num_seqs') and config.max_num_seqs is not None:
            model_kwargs["max_num_seqs"] = config.max_num_seqs
            logger.info(f"Setting max_num_seqs to {config.max_num_seqs}")
